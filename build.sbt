...
ThisBuild / githubWorkflowBuildPreamble ++= Seq(
  WorkflowStep.Run(
    ...
    commands = List("sudo apt-get install -y libutf8proc-dev libssl-dev")
  )
)
...
lazy val kyo = crossProject(JVMPlatform, JSPlatform, NativePlatform)
  .crossType(CrossType.Pure)
  .in(file("module/database/kyo"))
  .dependsOn(core)
  .settings(
    name := "pgmq4s-kyo",
...
lazy val examples = (project in file("examples"))
  .dependsOn(core.jvm, circe.jvm, anorm, doobie, skunk.jvm, slick, kyo.jvm)
