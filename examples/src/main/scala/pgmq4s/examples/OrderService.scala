...
trait OrderQueue[F[_]]:
  ...
  def read(vt: VisibilityTimeout, batchSize: BatchSize): F[List[Message.Inbound.Plain[OrderCreated]]]

object OrderQueue:
  def make[F[_]](queue: QueueName, client: PgmqClient[F]): OrderQueue[F] =
    new OrderQueue[F]:
      def send(event: OrderCreated): F[MessageId] = client.send(queue, Message.Outbound.Plain(event))
      def read(vt: VisibilityTimeout, batchSize: BatchSize): F[List[Message.Inbound.Plain[OrderCreated]]] =
        client.read[OrderCreated](queue, vt, batchSize)

class OrderService[F[_]: MonadThrow](queue: OrderQueue[F]):
  def publishAndFetch(event: OrderCreated): F[List[Message.Inbound.Plain[OrderCreated]]] =
    for
      _ <- queue.send(event)
