...
class DoobiePgmqClient[F[_]: Sync](xa: Transactor[F]) extends PgmqClient[F]:
  ...
  protected def sendRaw(queue: String, body: String): F[Long] =
    (sql"SELECT pgmq.send(${queue}, " ++ Fragment.const("$body::jsonb)"))
      .query[Long]
      .unique
      .transact(xa)

  protected def sendRaw(queue: String, body: String, delay: Int): F[Long] =
    (sql"SELECT pgmq.send(${queue}, " ++ Fragment.const("$body::jsonb,") ++ fr"${delay})")
      .query[Long]
      .unique
      .transact(xa)

  protected def sendBatchRaw(queue: String, bodies: List[String]): F[List[Long]] =
    val arr = bodies.toArray
    (sql"SELECT * FROM pgmq.send_batch(${queue}, " ++ Fragment.const("$arr::jsonb[])"))
      .query[Long]
      .to[List]
      .transact(xa)
  ...
  protected def sendBatchRaw(queue: String, bodies: List[String], delay: Int): F[List[Long]] =
    val arr = bodies.toArray
    (sql"SELECT * FROM pgmq.send_batch(${queue}, " ++ Fragment.const("$arr::jsonb[],") ++ fr"${delay})")
      .query[Long]
      .to[List]
      .transact(xa)
  ...
  protected def archiveBatchRaw(queue: String, msgIds: List[Long]): F[List[Long]] =
    val arr = msgIds.toArray
    sql"SELECT * FROM pgmq.archive(${queue}, ${arr})"
      .query[Long]
      .to[List]
      .transact(xa)
  ...
  protected def deleteBatchRaw(queue: String, msgIds: List[Long]): F[List[Long]] =
    val arr = msgIds.toArray
    sql"SELECT * FROM pgmq.delete(${queue}, ${arr})"
      .query[Long]
      .to[List]
      .transact(xa)
...
object DoobiePgmqClient:
  def apply[F[_]: Sync](xa: Transactor[F]): PgmqClient[F] =
    new DoobiePgmqClient[F](xa)
